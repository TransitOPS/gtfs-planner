  ...
  describe "parse/5" do
    ...
    test "complete input returns {:ok, ParsedEntity} keyed by natural-key value with exact row count" do
      ...
      assert entity.records_by_key == %{
               "L1" => %{level_id: "L1", level_name: "Platform"},
               "L2" => %{level_id: "L2", level_name: "Mezzanine"}
             }
    end

    test "keeps the upload filename for display and uses the canonical filename for diagnostics" do
      upload = %{filename: "user-upload.csv", content: "level_id,level_name\nL1,Platform\n"}

      assert {:ok, entity} =
               ParsedEntity.parse(upload, :level, "levels.txt", :level_id, fn row ->
                 {:ok, atomize(row)}
               end)

      assert entity.filename == "user-upload.csv"

      invalid_upload = %{upload | content: "level_id,level_name\nL1\n"}

      assert {:error, failure} =
               ParsedEntity.parse(invalid_upload, :level, "levels.txt", :level_id, fn row ->
                 {:ok, atomize(row)}
               end)

      assert failure.filename == "levels.txt"
      assert [%ParseError{file: "levels.txt"}] = failure.diagnostics
    end
    ...
    test "missing natural-key header returns {:error, ParseFailure} with :missing_natural_key_header" do
      ...
      assert [%ParseError{reason: :missing_natural_key_header, file: "levels.txt"}] =
               failure.diagnostics
    end
    ...
    test "duplicate natural key returns {:error, ParseFailure} with :duplicate_natural_key" do
      ...
      assert [%ParseError{reason: :duplicate_natural_key, row: 3, metadata: %{key: "L1"}}] =
               failure.diagnostics

      assert failure.preview_records_by_key == %{
               "L1" => %{level_id: "L1", level_name: "Platform"}
             }
    end
    ...
    test "semantic conversion error returns {:error, ParseFailure} with :semantic_row and bounded reason" do
      ...
      assert [%ParseError{reason: :semantic_row, row: 2, metadata: %{cause: :bad_level}}] =
               failure.diagnostics
    end
    ...
    test "an error on the final row after valid rows still prevents {:ok, ...}" do
      ...
      assert Enum.any?(failure.diagnostics, fn
               %ParseError{reason: :duplicate_natural_key} -> true
               _ -> false
             end)
    end
