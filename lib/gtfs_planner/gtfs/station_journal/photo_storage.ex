defmodule GtfsPlanner.Gtfs.StationJournal.PhotoStorage do
  ...
  @spec stage(Scope.t(), Ecto.UUID.t(), %{path: String.t()}) :: {:ok, inspected()} | {:error, atom()}
  def stage(%Scope{} = scope, photo_id, %{path: source_path}) when is_binary(source_path) do
    with {:ok, base_path, id} <- final_path(scope, photo_id),
         :ok <- File.mkdir_p(Path.dirname(base_path)),
      ...
      {:ok, Map.merge(result, %{path: staged_path, final_path: base_path <> result.extension, filename: filename})}
    end
  end
  ...
  def discard(%{path: path}) do
    _ = File.rm(path)
    :ok
  end

  @spec finalize(inspected()) :: :ok | {:error, term()}
  def finalize(%{path: path, final_path: final_path}) do
  ...
  def final_matches?(%{final_path: path} = staged) do
    case inspect(path) do
      {:ok, current} -> current.sha256 == staged.sha256 and current.content_type == staged.content_type
      {:error, _reason} -> false
    end
  end

  @spec inspect(String.t()) ::
          {:ok, %{content_type: String.t(), byte_size: pos_integer(), sha256: binary()}} | {:error, atom()}
  ...
  defp final_path(%Scope{} = scope, photo_id) do
    with true <- PathSafety.safe_path_component?(scope.organization_id),
         station_dir when is_binary(station_dir) <- PathSafety.stop_storage_dir(scope.station_stop_id),
  ...
  defp temporary_path(final_path, photo_id) do
    ...
    if PathSafety.ensure_within_root(Path.dirname(final_path), temp) == :ok, do: {:ok, temp}, else: {:error, :unsafe_path}
  end
              ...
              {:ok, inspected} ->
                case detected_type(inspected.prefix, inspected.tail) do
                  {:ok, content_type, extension} ->
                    {:ok, Map.put(inspected, :content_type, content_type) |> Map.put(:extension, extension)}
                  :error -> File.rm(staged); {:error, :invalid_image}
                end
              {:error, reason} -> File.rm(staged); {:error, reason}
            end
          {:error, _reason} -> File.close(input); {:error, :storage_error}
        end
  ...
  defp copy_chunks(input, output, hash, size, prefix, tail) do
    case IO.binread(input, @chunk_size) do
      :eof when size == 0 -> {:error, :empty_file}
      :eof when size > @max_bytes -> {:error, :payload_too_large}
      :eof -> {:ok, %{byte_size: size, sha256: :crypto.hash_final(hash), prefix: prefix, tail: tail}}
      {:error, _reason} -> {:error, :invalid_upload}
      chunk when is_binary(chunk) ->
        ...
        if next_size > @max_bytes do
          ...
          copy_chunks(input, output, :crypto.hash_update(hash, chunk), next_size, take_prefix(prefix, chunk), take_tail(tail, chunk))
        end
    end
  end
    ...
    case IO.binread(input, @chunk_size) do
      ...
      :eof ->
        case detected_type(prefix, tail) do
          {:ok, content_type, _extension} -> {:ok, %{content_type: content_type, byte_size: size, sha256: :crypto.hash_final(hash)}}
          :error -> {:error, :invalid_image}
        end
      chunk when is_binary(chunk) -> inspect_chunks(input, :crypto.hash_update(hash, chunk), size + byte_size(chunk), take_prefix(prefix, chunk), take_tail(tail, chunk))
      {:error, _reason} -> {:error, :invalid_upload}
    end
  end

  defp detected_type(<<0xFF, 0xD8, _::binary>>, tail) when byte_size(tail) >= 2 and binary_part(tail, byte_size(tail) - 2, 2) == <<0xFF, 0xD9>>, do: {:ok, "image/jpeg", ".jpg"}
  defp detected_type(@png_signature <> _rest, tail) when byte_size(tail) >= byte_size(@png_end) and binary_part(tail, byte_size(tail) - byte_size(@png_end), byte_size(@png_end)) == @png_end, do: {:ok, "image/png", ".png"}
  defp detected_type(_prefix, _tail), do: :error

  defp take_prefix(prefix, chunk), do: binary_part(prefix <> chunk, 0, min(8, byte_size(prefix <> chunk)))
