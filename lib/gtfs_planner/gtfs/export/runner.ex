defmodule GtfsPlanner.Gtfs.Export.Runner do
  @moduledoc "Temporary supervised owner for one fenced durable export build."
  ...
  def init(opts) do
    ...
    case ExportRuns.claim(organization_id, run_id, :build) do
      {:ok, %Run{} = run, generation, token} ->
        Process.flag(:trap_exit, true)
        worker = Keyword.get(opts, :worker_module, Worker)

        task =
          Task.Supervisor.async(GtfsPlanner.TaskSupervisor, fn ->
            worker.build(run, generation, token, ExportRuns.topic(run))
          end)
