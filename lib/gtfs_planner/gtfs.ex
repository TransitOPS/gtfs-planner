defmodule GtfsPlanner.Gtfs do
  ...
  When level_id is unchanged (or not present in attrs), delegates to `update_level/2`.

  ## Cascade targets

  - `stops.level_id` — any stop in the same org + version referencing the old level_id
  - `translations.record_id` where `table_name = "levels"` — any translation row
    in the same org + version with a matching record_id

  If a new entity is added that stores `level_id` as a string reference, it must
