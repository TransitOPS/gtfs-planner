...
import { selectSeededDiagramLevel } from "./station_diagram_helpers";
...
async function seededVersionId(page) {
  ...
  if (!id)
    throw new Error("Browser E2E Version is missing from the version switcher");
  return id;
}
...
async function openDrawer(page, locator, overlayId, attempts = 3) {
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    ...
    try {
      await page.waitForFunction(
        (id) =>
          document.getElementById(id)?.getAttribute("data-open") === "true",
        overlayId,
        { timeout: 4000 },
      );
      return;
...
async function expandAllReportDisclosures(page) {
  ...
  await page.waitForFunction(
    () =>
      document
        .getElementById("report-expand-all")
  ...
  for (const viewport of VIEWPORTS) {
    test(`report keeps identity, status, counts and actions at ${viewport.label}`, async ({
      ...
      expect(
        ...
        "the report page clips horizontally",
      ).toBe(true);
      ...
      const layout = await page.evaluate(() => {
        ...
        const longValue = Array.from(
          root.querySelectorAll("[phx-click='select_entity']"),
        ).find((el) =>
          el.textContent.includes("Northbound Interchange Concourse"),
        );
        ...
        return {
          ...
          longValue: longValue
            ? {
                scrollWidth: longValue.scrollWidth,
                clientWidth: longValue.clientWidth,
              }
            : null,
        };
      });
      ...
      expect(
        layout.longValue,
        "the long-named isolated node is not rendered",
      ).not.toBeNull();
      expect(
      ...
      expect(
        ...
        "no labelled overflow region is rendered",
      ).toBeGreaterThan(0);
      for (const region of layout.regions) {
        expect(
          region.label,
          "an overflow region has no accessible label",
        ).toBeTruthy();
        expect(
          region.tabindex,
          "an overflow region is not keyboard reachable",
        ).toBe("0");
        expect(
          region.overflowX,
          "an overflow region does not scroll locally",
        ).toBe("auto");
      }
    });
    ...
    test(`report disclosure and action targets meet 44 px at ${viewport.label}`, async ({
      ...
      expect(
        controls.length,
        "the report renders no controls to measure",
      ).toBeGreaterThan(0);
      for (const control of controls) {
        expect(
          control.height,
          `${control.id} is a ${control.height} px target`,
...
test.describe("Station report keyboard and motion contracts", () => {
  test("a disclosure is reachable by Tab, shows focus, toggles with Enter, and keeps focus", async ({
    ...
    expect(
      ...
      "the disclosure does not match :focus-visible",
    ).toBe(true);
    expect(
    ...
    const disclosures = await page.evaluate(() => {
      ...
      return buttons.map((button) => ({
        ...
        regionPresent: !!document.getElementById(
    ...
    for (const disclosure of disclosures) {
      ...
      expect(
        disclosure.regionPresent,
        `no region for ${disclosure.controls}`,
      ).toBe(true);
    }
  });

  test("reduced motion removes report transitions and skeleton animation", async ({
    page,
  }) => {
    ...
    const durations = await page.evaluate(() => {
      ...
      const control = document.querySelector(
        "#station-report-2 [data-report-control]",
      );
      return {
...
test.describe("Station report print evidence", () => {
  test("a newly loaded report prints complete evidence without chrome, controls, or drawers", async ({
    ...
    const untouched = await page.evaluate(
      () =>
        document.querySelectorAll("#station-report-2 [aria-expanded='true']")
          .length,
    );
    ...
    const printed = await page.evaluate(() => {
      ...
      const ids = (selector) =>
        Array.from(root.querySelectorAll(selector)).map((el) => el.id);
      const allVisible = (selector) =>
        Array.from(root.querySelectorAll(selector)).every((el) => visible(el));

      const controls = Array.from(
        root.querySelectorAll("[data-report-control]"),
      );
      ...
      return {
        ...
        h1: Array.from(root.querySelectorAll("h1")).map((h) =>
          h.textContent.trim(),
        ),
        ...
        subHeaderVisible: visible(
          document.getElementById("sub-header-wrapper"),
        ),
        visibleDialogs: Array.from(document.querySelectorAll("dialog")).filter(
          visible,
        ).length,
    ...
    expect(
      printed.checkRegions.length,
      "no failed-check detail is printed",
    ).toBeGreaterThan(0);
    expect(
      printed.checkAllVisible,
      "a failed-check detail is hidden in print",
    ).toBe(true);
    ...
    expect(
      printed.connectivityAllVisible,
      "a connectivity source is hidden in print",
    ).toBe(true);

    expect(
      printed.routeRegions.length,
      "no connectivity route is printed",
    ).toBeGreaterThan(0);
    expect(
      printed.routeAllVisible,
      "a connectivity route is hidden in print",
    ).toBe(true);
    expect(printed.stepRows, "no route step rows are printed").toBeGreaterThan(
      0,
    );
    ...
    expect(
      printed.visibleControls,
      "a report control survives into print",
    ).toBe(0);
    ...
    expect(
      printed.headerRepeats,
      "a table header does not repeat across pages",
    ).toBe(true);
...
test.describe("Station report stop drawer", () => {
  test("opens focused, stays one column at 320 px, acknowledges submit, and restores the opener", async ({
    ...
    const openerId = await page.evaluate(
      () =>
        document.querySelector("#station-report-2 [phx-click='select_entity']")
          .id,
    );
    ...
    const opened = await page.evaluate(() => {
      ...
      return {
        ...
        leftEdges: Array.from(
          new Set(
            controls.map((el) => Math.round(el.getBoundingClientRect().left)),
          ),
        ),
        ...
        pathwayFormPresent: !!document.getElementById(
          "report-pathway-edit-form",
        ),
      };
    });
    ...
    expect(
      opened.leftEdges,
      "the stop form is not one column at 320 px",
    ).toHaveLength(1);
    expect(opened.formScrollWidth).toBeLessThanOrEqual(opened.formClientWidth);
    expect(
      await bodyFitsViewport(page),
      "the drawer clips the page at 320 px",
    ).toBe(true);
    ...
    await watchPendingState(
      page,
      "#report-stop-edit-form button[type='submit']",
    );
...
test.describe("History tab keyboard contracts", () => {
  async function readTabs(page, entity) {
    return page.evaluate((prefix) => {
      const tabs = Array.from(
        document.querySelectorAll(`#${prefix}-tabs [role='tab']`),
      ).map((tab) => ({
        id: tab.id,
      ...
      const journalPanel = panel(`${prefix}-panel-journal`);
      ...
      return {
        ...
        journalHidden: journalPanel?.hasAttribute("hidden") ?? null,
        detailsLabelledBy: panel(`${prefix}-panel-details`).getAttribute(
          "aria-labelledby",
        ),
        historyLabelledBy: panel(`${prefix}-panel-history`).getAttribute(
          "aria-labelledby",
        ),
        journalLabelledBy:
          journalPanel?.getAttribute("aria-labelledby") ?? null,
        active: document.activeElement?.id ?? null,
      };
    }, entity);
  }
  ...
  async function assertTabContract(page, entity) {
    const hasJournal = entity === "stop" || entity === "pathway";
    const tabNames = hasJournal
      ? ["details", "history", "journal"]
      : ["details", "history"];
    const initial = await readTabs(page, entity);

    expect(initial.role).toBe("tablist");
    expect(initial.tabs.map((tab) => tab.id)).toEqual(
      tabNames.map((name) => `${entity}-tab-${name}`),
    );
    expect(initial.tabs.map((tab) => tab.controls)).toEqual(
      tabNames.map((name) => `${entity}-panel-${name}`),
    );
    expect(initial.detailsLabelledBy).toBe(`${entity}-tab-details`);
    expect(initial.historyLabelledBy).toBe(`${entity}-tab-history`);
    expect(initial.journalLabelledBy).toBe(
      hasJournal ? `${entity}-tab-journal` : null,
    );
    expect(initial.tabs.map((tab) => tab.selected)).toEqual(
      hasJournal ? ["true", "false", "false"] : ["true", "false"],
    );
    expect(initial.tabs.map((tab) => tab.tabindex)).toEqual(
      hasJournal ? ["0", "-1", "-1"] : ["0", "-1"],
    );
    expect(initial.detailsHidden).toBe(false);
    expect(initial.historyHidden).toBe(true);
    expect(initial.journalHidden).toBe(hasJournal ? true : null);
    for (const tab of initial.tabs) {
      expect(
        tab.height,
        `${tab.id} is a ${tab.height} px target`,
      ).toBeGreaterThanOrEqual(44);
    }
    ...
    await page.waitForSelector(`#${entity}-tab-history[aria-selected='true']`, {
      timeout: 10000,
    });

    const afterRight = await readTabs(page, entity);
    expect(afterRight.active).toBe(`${entity}-tab-history`);
    expect(afterRight.tabs.map((tab) => tab.tabindex)).toEqual(
      hasJournal ? ["-1", "0", "-1"] : ["-1", "0"],
    );
    expect(afterRight.detailsHidden).toBe(true);
    expect(afterRight.historyHidden).toBe(false);
    expect(afterRight.journalHidden).toBe(hasJournal ? true : null);

    if (hasJournal) {
      await page.keyboard.press("ArrowRight");
      await page.waitForSelector(
        `#${entity}-tab-journal[aria-selected='true']`,
        {
          timeout: 10000,
        },
      );

      const onJournal = await readTabs(page, entity);
      expect(onJournal.active).toBe(`${entity}-tab-journal`);
      expect(onJournal.tabs.map((tab) => tab.tabindex)).toEqual([
        "-1",
        "-1",
        "0",
      ]);
      expect(onJournal.detailsHidden).toBe(true);
      expect(onJournal.historyHidden).toBe(true);
      expect(onJournal.journalHidden).toBe(false);

      await page.keyboard.press("ArrowLeft");
      await page.waitForSelector(
        `#${entity}-tab-history[aria-selected='true']`,
        {
          timeout: 10000,
        },
      );
    ...
    // ArrowLeft returns to Details; Home selects the first tab and End the last.
    await page.keyboard.press("ArrowLeft");
    await page.waitForSelector(`#${entity}-tab-details[aria-selected='true']`, {
      timeout: 10000,
    });
    expect(await activeElementId(page)).toBe(`${entity}-tab-details`);

    const lastTab = hasJournal ? "journal" : "history";
    await page.keyboard.press("End");
    await page.waitForSelector(
      `#${entity}-tab-${lastTab}[aria-selected='true']`,
      {
        timeout: 10000,
      },
    );
    expect(await activeElementId(page)).toBe(`${entity}-tab-${lastTab}`);
  ...
  test("the stop drawer tabs stay synchronized under Arrow, Home and End", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 1280, height: 900 });
    ...
    await openDrawer(
      page,
      page
        .locator("button[phx-click='edit_child_stop']", { hasText: CHILD_STOP })
        .first(),
  ...
  test("the pathway drawer tabs stay synchronized under Arrow, Home and End", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 1280, height: 900 });
    await openDiagram(page);
  ...
  test("the level sidebar tabs stay synchronized under Arrow, Home and End", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 1280, height: 900 });
    await openDiagram(page);
    ...
    await openDrawer(
      page,
      page.locator("#edit-level-action"),
      "level-sidebar-overlay",
    );
  ...
  test("an edit is audited, rendered in agency time, and reverted with deterministic focus", async ({
    ...
    const openStopDrawer = () =>
      openDrawer(
        page,
        page
          .locator("button[phx-click='edit_child_stop']", {
            hasText: CHILD_STOP,
          })
          .first(),
        "child-stop-drawer-overlay",
    ...
    const originalName = await page.inputValue(
      "#child-stop-form input[name='stop_name']",
    );
    ...
    const history = await page.evaluate(() => {
      ...
      const after = nameChange?.querySelector(
        "[data-role='version-diff-after']",
      );
      const time = entry.querySelector("time[datetime]");

      return {
        entryCount: document.querySelectorAll("[data-role='history-entry']")
          .length,
        zoneNotes: [
          document.getElementById("history-timezone-stop")?.textContent.trim(),
          document
            .getElementById("history-utc-fallback-stop")
            ?.textContent.trim(),
        ].filter(Boolean),
        ...
        diffRows: entry.querySelectorAll("[data-role='version-diff-row']")
          .length,
        statusWord: entry
    ...
    expect(
      history.afterFits,
      "the audit value is clipped instead of wrapping",
    ).toBe(true);
    ...
    const preview = await page.evaluate(() => ({
      active: document.activeElement?.id ?? null,
      heading: document
        .getElementById("rollback-preview-heading-stop")
        ?.textContent.trim(),
    ...
    expect(preview.heading, "the preview does not name the stop").toContain(
      newName,
    );
    ...
    await page.locator("[data-history-entry-action]").first().click();
    await page.waitForSelector("#rollback-preview-confirm-stop", {
      timeout: 10000,
    });
    ...
    expect(
      await page.inputValue("#child-stop-form input[name='stop_name']"),
    ).toBe(originalName);
  });

  test("the history panel fits a 320 px drawer without clipping", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 320, height: 640 });
    await openDiagram(page);
    ...
    expect(
      await bodyFitsViewport(page),
      "the history panel clips the page at 320 px",
    ).toBe(true);
  });
});
